...
pub(crate) fn handoff_remote_connection(
    ...
    trace!(
        ...
        "remote-layer accept hook completed connection handoff"
    );
...
fn send_connection_handoff_request(
    ...
    trace!(
        ...
        peer_address = %request.peer_address,
        accepted_fd,
        "remote-layer sending accept handoff request"
    );
...
fn receive_connection_handoff_response(stream: &mut UnixStream) -> Result<AcceptHandoffResponse> {
    trace!("remote-layer waiting for accept handoff response");
    let mut decoder = SyncDecoder::<AcceptHandoffResponse, _>::new(stream.try_clone()?);
...
fn encode_connection_handoff_request(value: &AcceptHandoffRequest) -> Result<Vec<u8>> {
