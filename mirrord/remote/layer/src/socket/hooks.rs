...
/// before calling this helper, so ownership is explicit while we decide whether
/// to hand it off to the agent or fall back locally.
///
/// If this helper returns [`Detour::Success`], ownership of the accepted socket
/// fd is passed forward to the caller.
///
...
fn accept(sockfd: c_int, accepted_fd: &OwnedFd) -> Detour<c_int> {
    trace!(
        ...
        "remote-layer accept handoff path starting"
    );
        ...
        Ok(response) => match response.verdict {
            RemoteAcceptVerdict::Decline => {
                trace!(
                    sockfd,
                    accepted_fd = accepted_fd.as_raw_fd(),
                    "remote-layer accept handoff declined"
                );
            ...
            RemoteAcceptVerdict::Claim {
                ...
                Ok(placeholder_fd) => {
                    trace!(
                        ...
                        "remote-layer accept handoff claimed placeholder socket"
                    );
